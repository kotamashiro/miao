...
<!-- 在HTML中表达它们自身的字符。如果希望正确地显示预留字符，我们
必须在HTML源码中使用字符实体。
如：&nbsp; non-breaking space 160号空格
    ...
    &apos; 单引号-->
03. 计算机为什么使用二进制？
<!-- 模拟信号设备可以用通电和断开的方式表达0和1，足够简单 足够使用-->
04. 什么是 Unicode？如何表示，有什么作用？最通用的 Unicode 实现是？
<!--unicode即万国码，是国际组织制定的可以容纳世界上所有文字和符号的字符编码方案。 
用2个字节的十六进制数表示不同的字符。 
UTF-8 -->
...
32. 用文字描述如下选择器将选择哪些（个）元素
  ...
  <!--选中id为abc的div-->
  div:hover ul li > div {}
  <!-- div被hover时选中其后面的ul的后面的li里的div -->
  body :active {}
  <!-- 选中被用户激活时的body -->
  div:hover::after {}
  <!-- 选中div被hover时最后的伪元素 -->
  div:hover ::after {}
  <!-- 选中div被hover时其里面的子元素最后的伪元素 -->
  ::selection {}

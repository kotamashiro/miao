...
export default function ViewVote() {
  ...
  useEffect(() => {
    ...
    if (vote.deadline > new Date().toISOString()) {
      ...
      socket.emit('select room', vote.id)
      //收到了本投票下最新的信息
        ...
        return (
          ...
          <div key={idx}>
            <div onClick={() => voteOption(option.id, selected)}>
              ...
              <Progress percent={(option.Users.length / allUsers.length * 100).toFixed(2)} showInfo={false} />
            </div>
            {!vote.anonymous &&
              <div>
                {option.Users.map((user, idx) => {
                  return <span key={idx} className="avatar"
                    style={{ display: 'inline-block', width: 20, height: 20, borderRadius: '9999px', border: '1px solid' }}>
